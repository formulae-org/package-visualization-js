...
Visualization.createRectangle = async (createRectangle, session) => {
	let width = CanonicalArithmetic.getInteger(createRectangle.children[0]);
	if (width === undefined || width <= 0) {
		ReductionManager.setInError(createRectangle.children[0], "Invalid value");
		throw new ReductionError();
	}
	
	let height = CanonicalArithmetic.getInteger(createRectangle.children[1]);
	if (height === undefined || height <= 0) {
		ReductionManager.setInError(createRectangle.children[1], "Invalid value");
		throw new ReductionError();
	}
	
	let horzBaseline = CanonicalArithmetic.getInteger(createRectangle.children[2]);
	...
	let vertBaseline = CanonicalArithmetic.getInteger(createRectangle.children[3]);
...
Visualization.setFontSizeAndIncrement = async (setFontSizeAndIncrement, session) => {
	...
	let parameter = CanonicalArithmetic.getInteger(parameterExpression);
